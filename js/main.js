...
class List {
  constructor(url, container, list = listContext) {
    this.container = container; //содержит класс контейнера для товаров
    this.list = list; // словарь классов строка 213
    this.url = url;
    this.goods = []; //в данный массив пишем то что получаем с сервера
    this.allProducts = [];
...
class Item {
  constructor(el, img = 'https://via.placeholder.com/500x200') {
...
class Cart extends List {
  constructor(container = ".header__cart_overlay", url = "/getBasket.json") {
    super(url, container);
    this.getJson()
      .then(data => {
  ...
  _init() {
    document.querySelector('.header__cart_button').addEventListener('click', () => {
  ...
  render() {
    return `<div class="cart-item" data-id="${this.id_product}">
        ...
        <div class="right-block">
            ...
            <button class="del-btn" data-id="${this.id_product}">delete</button>
        </div>
        </div>`
  }
}
